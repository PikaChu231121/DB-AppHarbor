                ...
                <div class="content">
                    <div class="login-section">
                        <div class="login-form">
                            <h2 class="login_text"
                                style="font-family: 'Heiti SC'; font-size: 40px; font-weight: bolder; margin-top: 5px;">
                                登 录</h2>
                            <form @submit.prevent="login">
                                <div class="form-group">
                                    <label for="id"
                                        style="font-family: 'Hanyi Wenhei 85W', sans-serif; font-size: 20px;">商家ID</label>
                                    <input type="text" style="height: 40px; font-size: 15px;" id="id" v-model="id"
                                        required />
                                </div>
                                <div class="form-group">
                                    <label for="password"
                                        style="font-family: 'Hanyi Wenhei 85W', sans-serif; font-size: 20px;">密码</label>
                                    <input type="password" style="height: 40px;" id="password" v-model="password"
                                        required /><br><br><br>
                                </div>
                                <div class="button-row" style="margin-top: 9px;">
                                    <button type="button" class="login-button"
                                        style="font-size: 18px; margin-left: 0px; margin-top: -30px; width: 270px; font-family: 'Hanyi Wenhei 85W', sans-serif;"
                                        @click="goToLogin">登录</button>
                                </div>
                                <div class="button-row"
                                    style="font-family: 'Hanyi Wenhei 85W', sans-serif; margin-bottom: 20px;">
                                    <button type="button" class="secondary-button" @click="goToRegister">注册商家账号</button>
                                    <button type="button" class="secondary-button"
                                        @click="goToForgotPassword">修改密码</button>
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        login() {
            ...
            axios.post('http://localhost:5118/api/merchant/login', formData)
                .then(response => {
                    ...
                    this.alertMessage = `您好! 尊敬的 ${this.id} 商家, 欢迎来到 AppHarbor!`;
                    global.id = this.id;
                    setTimeout(() => {
                        this.$router.push('/MerchantWorkBanchPage'); // TODO: Merchant's route 尚未确定
...
.backwrapper {
    ...
    /*TODO: Create Merchant login_back.svg*/
